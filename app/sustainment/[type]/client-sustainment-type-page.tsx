...
export function ClientSustainmentTypePage({ type }: { type: string }) {
  ...
  const getTasksForLOEAndType = (loeId: string) =>
    tasks.filter(
      task =>
        task.loeId === loeId &&
        (task.category === "sustainment" || !task.category) &&
    ...
    <div className="min-h-screen bg-gray-50">
      <main className="container mx-auto px-4 py-8 max-w-7xl">
        <h1 className="text-4xl font-bold text-gray-900 mb-2">
          Sustainment - {typeLabels[type as keyof typeof typeLabels]}
        </h1>
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>
              {typeLabels[type as keyof typeof typeLabels]} health status across all sustainment LOEs
            </CardDescription>
          </CardHeader>
          <CardContent>
